    ...
    public class PinchManager : MonoBehaviour
    {
        ...
        void Update()
        {
            if (TryEnsureInitialized())
            {
                ...
                {
                    var spawnedObj = Instantiate(m_RightSpawnPrefab, m_RightMidPoint, Quaternion.identity);
                ...
                {
                    var spawnedObj = Instantiate(m_LeftSpawnPrefab, m_LeftMidPoint, Quaternion.identity);
                    float scaleFactor = 0.1f;
                    spawnedObj.transform.localScale = new Vector3(scaleFactor, scaleFactor, scaleFactor);
                }
            }
